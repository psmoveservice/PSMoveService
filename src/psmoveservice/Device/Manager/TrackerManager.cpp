...
TrackerManagerConfig::config2ptree()
{
	...
	pt.put("optical_tracker_skip_frames", optical_tracker_skip_frames);
...
TrackerManagerConfig::ptree2config(const boost::property_tree::ptree &pt)
{
    ...
    if (version == TrackerManagerConfig::CONFIG_VERSION)
    {
		...
		optical_tracker_skip_frames = pt.get<int>("optical_tracker_skip_frames", optical_tracker_skip_frames);
