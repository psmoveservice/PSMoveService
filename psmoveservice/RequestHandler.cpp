...
static void handle_request__acquire_psmove(const RequestContext &context, PSMoveDataFrame::Response *response);
static void handle_request__release_psmove(const RequestContext &context, PSMoveDataFrame::Response *response);
...
ResponsePtr RequestHandler::handle_request(RequestPtr request)
{
    ...
    switch (request->type())
    {
        case PSMoveDataFrame::Request_RequestType_GET_ACTIVE_PSMOVE_COUNT:
            ...
            break;
        case PSMoveDataFrame::Request_RequestType_ACQUIRE_PSMOVE:
            handle_request__acquire_psmove(context, response);
            break;
        case PSMoveDataFrame::Request_RequestType_RELEASE_PSMOVE:
            handle_request__release_psmove(context, response);
            break;
...
static void handle_request__acquire_psmove(
    const RequestContext &context, 
    PSMoveDataFrame::Response *response)
{
    // TODO
    response->mutable_response_general()->set_code(PSMoveDataFrame::Response_ResultCode_RESULT_OK);
}

static void handle_request__release_psmove(
    const RequestContext &context,
    PSMoveDataFrame::Response *response)
{
    // TODO
